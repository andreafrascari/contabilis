...
/**
 * Questa funzione genera automaticamente tutte le proforma in scadenza.
Il sistema parte dal "calendario di fatturazione ", riempito automaticamente per ogni cliente ad inizio anno in base a:

    data prima proforma
    numero rate

    Da questo calendario seleziona  i clienti che hanno fatture "in scadenza", ovvero con data minore o uguale alla data odierna.

    Per ognuno di questi clienti, vengono reperite le pratiche NON SOSPESE dell'anno contabile corrente.

    Se ci sono pratiche, viene creata una proforma così composta:
    importo:
        ogni pratica "a forfait" concorre all'importo complessivo per il suo costo diviso il numero di rate per il cliente;
        ogni pratica "a prestazione" CHIUSA e NON FATTURATA concorre all'importo complessivo per il suo costo intero;
        ogni pratica "a ore" concorre all'importo complessivo in base alle ore di lavoro segnate sulle relative attività NON FATTURATE e al costo orario dell'operatore che le ha svolte (le attività cambiano poi stato in "fatturate")
    spese anticipate:
        somma di tutte le  spese anticape attribuite ad ogni pratica

    Alla proforma viene asseganto stato "in attesa", numero (?), viene associata alla relativa entry del calendario di fatturazione (in modo che questa entry non ricompaia alla prossima generazione massiva) e viene registrata
 * @author afrascari
 *
 */
public class GeneraProformaMethod extends DefaultMethod 
{
	 ...
	 */
	private static final Logger logger = Logger.getLogger(GeneraProformaMethod.class);
	...
	public GeneraProformaMethod(DefaultModule parentModule,String[] defaultParameters)
	{
	...
	public String doMethod(HttpServletRequest request,HttpServletResponse response) throws SerenaException
	{
		...
		try	{
			...
			ArrayList<Attivita> attivitaDaAggiornare = new ArrayList<Attivita>(); // lista attivita di cui aggiornare lo stato in "fatturate"
			ArrayList<Pratica> praticheDaAggiornare = new ArrayList<Pratica>(); // lista attivita di cui aggiornare lo stato in "fatturate"
			ArrayList<SpesaAnticipata> speseDaAggiornare = new ArrayList<SpesaAnticipata>(); // lista attivita di cui aggiornare lo stato in "fatturate"
			String anno_contabile = request.getParameter(PARAM_ANNO);
			if (anno_contabile==null || anno_contabile.isEmpty())	{
				// anno corrente
				anno_contabile = new Integer(new SerenaDate().getYear()).toString();
			}
			logger.info("Invocata generazione massiva proforma per anno contabile "+anno_contabile );
			...
			List<Element> itemList = itemCandidati.selectNodes(".//"+CalendarioFatturazione.INSTANCE_NAME);
			ProForma p = null;
			...
			for (Element anItem: itemList)	{
				CalendarioFatturazione cf = binder.createCalendarioFatturazione(anItem);
				int rataCorrente = cf.getRata().intValue();
				Cliente c = new Cliente();
				c.setId(cf.getPer_cliente().getId());
				c.setCliente(cf.getPer_cliente().getCliente());	
				String nrate = anItem.elementText(CalendarioFatturazione.SLOT_SU_RATE);
				p = new ProForma();
				p.setAnno_contabile(anno_contabile);
				p.setData(cf.getData());
				p.setEntry_calendario(cf);
				p.setInverse_of_proforma(c);
				p.setCompetenza(ProForma.COMPETENZA__CONTABILIS);	// default
				p.setRa(false);
				p.setRiv_prev(false);				
				p.setStato_proforma(ProForma.STATO_PROFORMA__DA_VALIDARE);
				// quali pratiche/attività/spese anticipate vanno fatturate?
				Element pratiche = getPratiche(request,anno_contabile,c.getId());
				if (pratiche!=null)	{
					// ogni pratica "movimentata" diverrà una voce proforma
					List<Element> praticheCandidate = pratiche.selectNodes(".//"+Pratica.INSTANCE_NAME);
					ArrayList<MyVoceFattura> voci = new ArrayList<MyVoceFattura>(); 
					// si utilizza per calcolare i totali di importi e descrizione di tutte le spese di tutte le attivita di tutte le pratiche:
					MySpesaAnticipata virtualSpesa4Total = new MySpesaAnticipata();
					virtualSpesa4Total.init();
					// cicle sulle pratiche (pratica) candidate alla proforma corrente (p)
					for (Element praticaElem: praticheCandidate)	{
						Pratica pratica = binder.createPratica(praticaElem);
						MyVoceFattura vf=null;
						try	{
							vf =getMyVoceFattura(pratica, new Integer(nrate).intValue(),attivitaDaAggiornare, praticheDaAggiornare, rataCorrente);
							getSommaSpeseAnticipate(pratica,virtualSpesa4Total,speseDaAggiornare);
							if (vf!=null){
								voci.add(vf);
							} 
						} catch (Exception e) {
							String msg = "Ignorato cliente <strong>"+c.getCliente()+"</strong> nonstante la scadenza del "+cf.getData()+" per <strong>forfait non prezzato</strong>";	
					...
					if (voci.isEmpty())	{
						// potrebbero essere spese anticpate! fabio:Va bene così, se anticipiamo delle spese significa che abbiamo un incarico, quindi prima o poi ci sarà qualcosa da fatturare.
						String msg = "Nessuna pratica da fatturare per il cliente <strong>"+c.getCliente()+"</strong> nonstante la scadenza del "+cf.getData();
						if (virtualSpesa4Total.getImporto()!=null && virtualSpesa4Total.getImporto().floatValue()>0)	{
							msg += " ... ci sarebbero pero' " + virtualSpesa4Total.getImporto().floatValue() + " di spese anticipate!";
						}
						mess = addOutputEntry(msg,mess);
						logger.info(msg);
						continue;
					} else {
						ArrayList<VoceFattura> processedVoci =  aggregaVociForfait(voci,anItem);
						p.setVoci_proforma(processedVoci);
						p.setSpese_anticipate_fattura(virtualSpesa4Total.getImporto());
						p.setSpese_anticipate_desc(virtualSpesa4Total.getOggetto());
					}
				}  else {
					String msg = "Nessuna pratica da fatturare per il cliente <strong>"+c.getCliente()+"</strong> nonstante la scadenza del "+cf.getData();
					mess = addOutputEntry(msg,mess);
				...
				/* queste? dipendono dalla "competenza" (Tullini o SRL ... ma qui non la conosco)
				p.setRa(ra);
				p.setRiv_prev(riv_prev);
				*/
				p.setNumero(MyProForma.getNextNumberContabilis(anno_contabile,request));
				try	{
					...
					if (aNewID!=null)	{
						...
						String msg="Proforma ID "+aNewID+ " registrata correttamente: <a href=\"?q=object/detail&amp;p="+ProForma.INSTANCE_NAME+"/_a_ID/_v_"+aNewID+"\" title=\"vai alla proforma\">vai</a>";
						mess = addOutputEntry(msg,mess);
						try	{
							if (attivitaDaAggiornare.size()>0)	{
								aggiornaStatoAttivita(attivitaDaAggiornare, request);
							}
							if (praticheDaAggiornare.size()>0)	{
								aggiornaStatoPratica(praticheDaAggiornare, request);
							}
							if (speseDaAggiornare.size()>0)	{
								aggiornaStatoSpese(speseDaAggiornare, request);
							}
						} catch (Exception e) {
					...
					String msg = "Proforma cliente <strong>"+c.getCliente()+"</strong> non registrata a causa di un errore/duplicazione;";
					mess = addOutputEntry(msg,mess);
				}
			}			
			if (mess.isEmpty())
	...
	/**
	 * Aggrega le voci a forfait in un unico item con descrizione presa da parametro (acconto o saldo)
	 * @param voci
	 * @param entryCalendario
	 */
	private ArrayList<VoceFattura>  aggregaVociForfait(ArrayList<MyVoceFattura> voci, Element entryCalendario) throws SerenaException {
		String accontoDesc =ModuleParameterConfiguration.getInstance().getValue("helpgest","VOCE_FORFAIT_ACCONTO");
		String saldoDesc =ModuleParameterConfiguration.getInstance().getValue("helpgest","VOCE_FORFAIT_SALDO");
		String nrate = entryCalendario.elementText(CalendarioFatturazione.SLOT_SU_RATE);
		String dataEntry = entryCalendario.elementText(CalendarioFatturazione.SLOT_DATA);
		int mese = new SerenaDate(dataEntry).getMonth();
		
		// saldo o acconto? acconto se 1 sola rata oppure mese dicembre
		String forfaitDesc = ("1".equals(nrate) || mese==12)?saldoDesc:accontoDesc;
		...
		for (MyVoceFattura vf: voci)	{
			if (vf.isFromForfait())	{
				...
				forfait.setRif_pratica(vf.getRif_pratica());	// il rif non e' preciso perche' e' quello dell'ultima .... ma e' lo stesso
			} else {
	 ...
	 */
	private void aggiornaStatoAttivita(ArrayList<Attivita> daAggiornare, HttpServletRequest request)  throws SerenaException {
		try	{
			for (Attivita anAtt: daAggiornare)	{
				...
				Element anItem = currentElement.addElement(Attivita.SLOT_STATO_ATTIVITA);
				anItem.setText(anAtt.getStato_attivita().toString());
				anItem = currentElement.addElement(Attivita.SLOT_MINUTI_ULTIMA_FATTURA);
				...
				String[] messages2={"",""};
				int res = ConstantsXSerena.getXserenaRequestResult(data, messages2,SpesaAnticipata.INSTANCE_NAME);
				if (res==ConstantsXSerena.XSERENA_RESULT_ERROR)
				{
					logger.error("cannot update ore fatturate in attivita: "+messages2[0]);
					throw new SerenaException("cannot update ore fatturate in attivita: "+messages2[0]);
				}			
			}
		} catch (Exception e) {
			logger.error("cannot update ore fatturate attivita"+e.getMessage());
			throw new SerenaException("cannot update  ore fatturate attivita"+e.getMessage());
		}
		
	}
	
	private void aggiornaStatoSpese(ArrayList<SpesaAnticipata> daAggiornare, HttpServletRequest request)  throws SerenaException {
		try	{
			// 1 tutte le spese di queste attivita fatturate
			Element currentElement = ObjectUtils.getXserenaRequestStandardHeader(request.getSession().getId(), ConstantsXSerena.ACTION_SET, SpesaAnticipata.INSTANCE_NAME);
			currentElement.addAttribute(ConstantsXSerena.ATTR_OPERATION, ConstantsXSerena.VAL_UPDATE);
			Element condition = currentElement.addElement(ConstantsXSerena.TAG_CONDITION);
			condition = condition.addElement("ID");
			condition.addAttribute(ConstantsXSerena.ATTR_OPERATOR, ConstantsXSerena.VAL_IN);
			String inClause = "";
			...
			currentElement = currentElement.addElement(SpesaAnticipata.SLOT_FATTURATA);
			currentElement.setText("1");
			Document data = ApplicationLibrary.setData(currentElement.getDocument(),request,true);
			String[] messages={"",""};
			int res = ConstantsXSerena.getXserenaRequestResult(data, messages,SpesaAnticipata.INSTANCE_NAME);
			if (res==ConstantsXSerena.XSERENA_RESULT_ERROR)
			{
				logger.error("cannot update stato fatturate in spese anticipate: "+messages[0]);
				throw new SerenaException("cannot update stato fatturate in spese anticipate: "+messages[0]);
			}	
		} catch (Exception e) {
			logger.error("cannot update stato fatturate in spese anticipate"+e.getMessage());
			throw new SerenaException("cannot update stato fatturate in spese anticipate"+e.getMessage());
		}
		
	}
	
	private void aggiornaStatoPratica(ArrayList<Pratica> daAggiornare, HttpServletRequest request)  throws SerenaException {
		try	{
			...
			if (res==ConstantsXSerena.XSERENA_RESULT_ERROR)
			{
				logger.error("cannot update stato fatturate in pratiche: "+messages2[0]);
				throw new SerenaException("cannot update stato fatturate in pratiche: "+messages2[0]);
			}			
		} catch (Exception e) {
			logger.error("cannot update stato fatturate in pratiche"+e.getMessage());
			throw new SerenaException("cannot update stato fatturate in pratiche"+e.getMessage());
		}
	 ...
	 * @throws SerenaException
	 */
	private MyVoceFattura getMyVoceFattura(Pratica pratica, int nRate, ArrayList<Attivita> attDaAggiornare, ArrayList<Pratica> praDaAggiornare, int rataCorrente) throws SerenaException {
		MyVoceFattura vf = new MyVoceFattura();
		if (pratica.getTitolo_per_fattura()!=null && !pratica.getTitolo_per_fattura().isEmpty())	{
			vf.setOggetto(pratica.getTitolo_per_fattura());
		} else {
			vf.setOggetto(pratica.getTitolo());
		}
		if (vf.getOggetto()==null || vf.getOggetto().isEmpty())	{
			String message ="Voce fattura senza oggetto a causa di pratica senza titolo!!!" + pratica.getId()+ " metto titolo fittizio...";
		...
		if (pratica.getTipo().equals(Pratica.TIPO__A_FORFAIT))	{
			...
			if (pratica.getPrezzo()==null)	{
				String message ="Pratica a forfait senza prezzo: cliente ignorato causa pratica forfait: "+pratica.getId(); 
				logger.fatal(message);
			...
			if (rataCorrente==nRate)	{
				praDaAggiornare.add(pratica); // le pratiche a forfait diventano FATTURATE solo all'ULTIMA RATA 
			}
			...
			if (pratica.getStato().equals(Pratica.STATO__CHIUSA))	{
				...
				if (pratica.getPrezzo()==null)	{
					String message ="Pratica a prestazione senza prezzo: considero 0: "+pratica.getId(); 
					logger.warn(message);
					importo = new Float(0);
				} else	{
					importo = new Float(pratica.getPrezzo()).floatValue();
				}
				praDaAggiornare.add(pratica); // le pratiche a prestazione diventano SEMPRE FATTURATE 
			} else {
					logger.warn("Pratica " + pratica.getId() + "("+pratica.getTitolo()+") a prestazione viene ignorata perche non CHIUSA");
					return null;
			}
		} else if (pratica.getTipo().equals(Pratica.TIPO__A_ORA))	{
			// ad ora: calcolo in base alle ore lavorate * costi orari operatori attività
			...
			for (Attivita att: attivita)	{
				...
				if (costo>0){
					...
					if (pratica.getStato().equals(Pratica.STATO__CHIUSA))	{
						praDaAggiornare.add(pratica);	// le pratiche a ore diventano FATTURATE SOLO se erano chiuse
					}
				}
			}
		} else{
			String mess ="Pratica " + pratica.getId() + "("+pratica.getTitolo()+") ha tipo sconosciuto";
			logger.error(mess);
	...
	/**
	 * Calcola il prezzo applicato per l'attività come somma dei minuti di tutte le sessioni di lavoro
	 * * costo orario operatore / 60
	 * @param att
	 * @return
	 * @throws SerenaException
	 */
	private int getPrezzoPerAttivita(Attivita att) throws SerenaException {
		try	{
			...
			ArrayList<LavoroSuAttivita> sessioniDiLavoro = att.getSessioni_di_lavoro();
			if (sessioniDiLavoro.size()>0){
				...
				logger.trace("Attivita "+att.getId() + " minuti di lavoro: "+minuti + " di cui "+minutiFatturati + " gia fatturate");
				// aggiorni minuti fatturati attivita
				att.setMinuti_ultima_fattura(att.getStato_attivita());	// memo valore precedente di minuti fatturati per rollback
				att.setStato_attivita(minuti);
				// sottraggo i minuti gia fatturati:
				minuti-=minutiFatturati;
				ret =  costoHF*minuti/60;
			} else	{
				logger.warn("Attivita "+att.getId() + " senza sessioni di lavoro");
			}
			return ret;
		} catch (Exception e) {
			String msg = "Attivita "+att.getId() + " impossibile determinare il costo quindi non entra in proforma "+e.getMessage();
			logger.error(msg);
	 ...
	 * @return
	 */
	private void getSommaSpeseAnticipate(Pratica pratica, MySpesaAnticipata va, ArrayList<SpesaAnticipata> daAggiornare) {
		ArrayList<Attivita> attivita = pratica.getAttivita();
				...
				if (spese.size()>0){
					for (SpesaAnticipata sa: spese){
						if (!sa.getFatturata())	{
							daAggiornare.add(sa);
							any=true;
							costoSpeseAnticipate+=new Float(sa.getImporto()).floatValue();
							logger.trace("Calcolo spese anticipate: aggiungo " + sa.getOggetto() + ": " + sa.getImporto() + " totale = "+costoSpeseAnticipate);
							String data = new SerenaDate(sa.getData()).toString();
							spesaDesc.append("<strong>"+sa.getOggetto()+"</strong> ("+data+"): &euro; "+new UtilsProvider().formatImporto(sa.getImporto())+";<br />");
					}
				}
			}
		}
//		if (costoSpeseAnticipate>0){	// nel caso sfortunato di importi +/- che si annullano a vicenda, non metteva nulla!
		if (any){
	...
	/**
	 * Torna la lista delle pratiche APERTE OPPURE CHIUSE CON DATA CHIUSURA (per evitare quelle "in attesa") del cliente per l'anno contabile CORRENTE
	...
	private Element getPratiche(HttpServletRequest request,
		...
		try {
			...
			Element item =currentElement.addElement(Pratica.SLOT_CLIENTE_PRATICA);
	...
	private String insertProforma(Element theQuery, HttpServletRequest request) throws SerenaException {
		String newID = null;
		try	{
			...
			if (res==ConstantsXSerena.XSERENA_RESULT_ERROR)
			{
				...
				throw new SerenaException("cannot insert prpforma: "+messages2[0]);
			}
			else if (res==ConstantsXSerena.XSERENA_RESULT_SUCCESS)	{
				newID = messages2[1];
			}		
		} catch (ObjectAlreadyExistsException e)
		{
			...
			throw new SerenaException("cannot insert prpforma - oggetto esistente  "+e.getMessage());
	...
	/**
	 * GLi item candidati sono le entry del calendario ancora senza proforma assiciata con data <= oggi
	 * @param request
	 * @return
	 * @throws SerenaException
	 */
	protected Element getItemCandidati(HttpServletRequest request) throws SerenaException	{
		try {
			Element currentElement = ObjectUtils.getXserenaRequestStandardHeader(request.getSession().getId(), ConstantsXSerena.ACTION_GET, CalendarioFatturazione.INSTANCE_NAME);
			currentElement.addAttribute(ConstantsXSerena.ATTR_OPERATION, ConstantsXSerena.VAL_SELECT);
			...
			currentElement.addAttribute(ConstantsXSerena.ATTR_FORCED_DATASOURCE, "preview-generazione");
			/*
			currentElement=currentElement.addElement(ConstantsXSerena.TAG_CONDITION);
			Element item =currentElement.addElement("proforma_emessa");
			item.setText(ConstantsXSerena.VAL_NULL);
			item =currentElement.addElement("data");
			item.addAttribute(ConstantsXSerena.ATTR_OPERATOR,ConstantsXSerena.VAL_LESS_THAN);
			...
			*/
			...
			if (res==ConstantsXSerena.XSERENA_RESULT_ERROR)
			{
				logger.error("getItemCandidati for proforma has resulted into sql error:"+messages[0]);
				throw new SerenaException(messages[0],"generic_sql_error");
			}
			else if (res==ConstantsXSerena.XSERENA_RESULT_EMPTY)
			{
			...
			logger.error("getItemCandidati for proforma has resulted into sql error"+e.getMessage());
			throw new SerenaException(e.getMessage());
		} catch (DataManagerException e) {
			logger.error("getItemCandidati for proforma has resulted into sql error"+e.getMessage());
			throw new SerenaException(e.getMessage());
	...
	protected String getIva() throws SerenaException	{
		try	{
			return ModuleParameterConfiguration.getInstance().getValue("helpgest","IVA_DEFAULT");
		} catch (Exception e) {
